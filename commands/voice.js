module.exports = {
  ...
  permission : {
	...
	init : function(){
		...
		client.on('channelUpdate', (before, after) => this.channelUpdate(before, after));
		...
		// if(!channel)
		channel = this.channelCreate(category);
	...
	/**
	 * Выполняет действие, указанное в параметре
	 *
	 * @param {Message} msg
	 * @param {String}  param Параметр команды
	 */
	call : async function(msg, params){ this.action(msg, params[0][0]); },
	slash : async function(int){ this.action(getMsg(int), int.data.options[0].name[0]); },
	action : async function(msg, param){
		if(param == 'r') return this.reset(msg);
		if(param == 'f') return this.fix(msg);
	...
	update : function(before, after){
		...
		if(channel.after.id == this.channel.id) return this.create(after);
	...
	create : async function(data){
		const user = DB.query('SELECT * FROM users WHERE id = ?', [data.member.user.id]);
		...
		const voice = user.length ? JSON.parse(user[0].voice_data) : this.new(data);
		let options = {
		...
		const text = await data.guild.channels.create(voice.name, {
			reason : 'По требованию ' + member2name(data.member, 1),
			parent : this.categoryChannel.id,
	...
	/**
	 * Ивент обновления канала. Необходим для обновления названия текстового канала
	 *
	 * @param {GuildChannel} before
	 * @param {GuildChannel} after
	 */
	channelUpdate : function(before, after){
